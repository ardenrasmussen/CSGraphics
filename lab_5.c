...
void DrawVector(double point[3], double vec[3], double color[3])
{
  double end[3] = { (point[0] + vec[0]), (point[1] + vec[1]), (point[2] + vec[2]) };
  double x[2], y[2];
  x[0] = ((((double)WIDTH / 2.0) / tan(HALF)) * (point[0] / point[2])) + ((double)WIDTH / 2.0);
  x[1] = ((((double)WIDTH / 2.0) / tan(HALF)) * (end[0] / end[2])) + ((double)WIDTH / 2.0);
  y[0] = ((((double)HEIGHT / 2.0) / tan(HALF)) * (point[1] / point[2])) + ((double)HEIGHT / 2.0);
  y[1] = ((((double)HEIGHT / 2.0) / tan(HALF)) * (end[1] / end[2])) + ((double)HEIGHT / 2.0);
  G_rgb(color[0], color[1], color[2]);
  G_line(x[0], y[0], x[1], y[1]);
}
...
void SetColor(Polygon poly)
{
  ...
  Normalize(center, vu);
  Invert(vu, vu);
  /* lu[0] = center[0] - light[0]; */
  lu[0] = light[0] - center[0];
  lu[1] = light[1] - center[1];
  lu[2] = light[2] - center[2];
  /* lu[1] = center[1] - light[1]; */
  /* lu[2] = center[2] - light[2]; */
  /* Invert(lu, lu); */
  Normalize(lu, lu);
  FindNormal(poly, nu);
  if (INVERSE == 1) {
  ...
  /* printf("Vu:     [%+lf, %+lf, %+lf]\n", vu[0], vu[1], vu[2]); */
  printf("%lf\n", intensity);
  /* DrawVector(center, vu, c); */
  /* c[1] = 1; */
  /* DrawVector(center, lu, c); */
